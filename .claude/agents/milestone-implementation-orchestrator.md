...
[6] implement-issues (loop over implementation_order)
  ...
  │       (and its section checkbox once every issue in that section is done)
...
- **Ownership**: the milestone agent owns the PR body exclusively. The issue orchestrator does **not** append `Closes #N` or summaries to the PR body in milestone mode — it only reports its summary back to this orchestrator, which updates the checkboxes.
